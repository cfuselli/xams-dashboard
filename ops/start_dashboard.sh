...
PORT=${XAMS_DASH_PORT:-8070}
ENV_ACTIVATE=${XAMS_DASH_ENV_ACTIVATE:-"source /data/xenon/xams_v2/setup.sh"}

screen -S "$SCREEN_NAME" -X quit || true
screen -S "$SCREEN_NAME" -dm bash -lc "cd '$APP_DIR' && $ENV_ACTIVATE && export XAMS_DASH_HOST='$HOST' XAMS_DASH_PORT='$PORT' XAMS_CONFIG_FILE=~/.xams_config && python -m v2.app"
